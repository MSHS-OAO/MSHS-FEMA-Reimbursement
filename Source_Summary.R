Source_Summary <- function(data){
  ...
  #Begin summarization
  Department <- data %>%
    group_by(PAYROLL,HOME.SITE,WRKD.LOCATION,HOME.LOCATION,DPT.WRKD,DPT.HOME,WRKD.DESCRIPTION,HOME.DESCRIPTION,J.C,J.C.DESCRIPTION,PAY.CODE,END.DATE) %>%
  ...
  Summary <- left_join(Department,PayCycle,by = c("END.DATE" = "Date.1")) %>%
    select(PAYROLL,HOME.SITE,WRKD.LOCATION,HOME.LOCATION,DPT.WRKD,DPT.HOME,WRKD.DESCRIPTION,HOME.DESCRIPTION,J.C,J.C.DESCRIPTION,PAY.CODE,End.Date,HOURS,EXPENSE) %>%
    group_by(PAYROLL,HOME.SITE,WRKD.LOCATION,HOME.LOCATION,DPT.WRKD,DPT.HOME,WRKD.DESCRIPTION,HOME.DESCRIPTION,J.C,J.C.DESCRIPTION,PAY.CODE,End.Date) %>%
