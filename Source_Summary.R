Source_Summary <- function(data){
  ...
  System_Paycode <- System_Paycode %>%
    mutate(PAY.CODE = str_trim(`Hospital Pay Code`)) 
  ...
  #Begin summarization
  Department <- data %>%
    group_by(WRKD.SITE,HOME.SITE,WRKD.LOCATION,HOME.LOCATION,DPT.WRKD,DPT.HOME,WRKD.DESCRIPTION,HOME.DESCRIPTION,J.C,J.C.DESCRIPTION,PAY.CODE,END.DATE) %>%
  ...
  Summary <- left_join(Department,PayCycle,by = c("END.DATE" = "Date.1")) %>%
    select(WRKD.SITE,HOME.SITE,WRKD.LOCATION,HOME.LOCATION,DPT.WRKD,DPT.HOME,WRKD.DESCRIPTION,HOME.DESCRIPTION,J.C,J.C.DESCRIPTION,PAY.CODE,End.Date,HOURS,EXPENSE) %>%
    group_by(WRKD.SITE,HOME.SITE,WRKD.LOCATION,HOME.LOCATION,DPT.WRKD,DPT.HOME,WRKD.DESCRIPTION,HOME.DESCRIPTION,J.C,J.C.DESCRIPTION,PAY.CODE,End.Date) %>%
